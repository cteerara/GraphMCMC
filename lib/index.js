...
// Get Bridges
function GetBridges(S){
	//Get a S.VertexNumber-by-S.VertexNumber boolean array indicating which edge is a bridge
	var Stmp = S
	var B = new Array(S.VertexNumber)
	for (var k=0;k<S.VertexNumber;k++) B[k] = new Array(S.VertexNumber)

	for (var i=0;i<S.VertexNumber;i++){
		for (var j=0;j<S.VertexNumber;j++){
			B[i][j] = false
		}
	}
