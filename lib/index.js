...
function GetCandidate(S){
  ...
  }else if ( BridgeNumber(S) == S.EdgeNumber){
    //Add edge
...
//  console.log('S0')
//  console.log(S0.AdjMatrix)
//  console.log('Scand')
//  console.log(Scand.AdjMatrix)
//  console.log('RSD',RSD)
//  console.log('q1to0',q1to0)
//  console.log('q0to1',q0to1)
//  console.log('Acceptance probability')
//  console.log(AcptProb)
//  console.log(u<AcptProb)
//  console.log(' ')
//  console.log(' ')
...
function Main(){
  ...
  StateArray[0] = CloneState(S0)
  var u = 0
  for (var i=1;i<nsamples+1;i++){
    u = Math.random()
    var Scand = GetCandidate(StateArray[i-1])
    if ( MH(StateArray[i-1],Scand,r,T,u) ){
      StateArray[i] = CloneState(Scand);
    } else {
      StateArray[i] = CloneState(StateArray[i-1])
    }
  }
  ...
  for (i=1;i<nsamples+1;i++){
    ...
    var Snew = CloneState(StateArray[i])
    var SLlen = StateList.length
    console.log(SLlen)
    for (var j=0;j<SLlen;j++){
      var StateInList = CloneState(StateList[j])
      if ( StateCmp(Snew,StateInList) ){
  ...
  console.log(NumOfOccur.length)
  //console.log(NumOfOccur)
}
...
module.exports = {State, getMinDist, getLenSP, getTheta, getRSD, IsConnected, CloneState, BridgeNumber, GetProposalDist, GetCandidate, MH, StateCmp };
