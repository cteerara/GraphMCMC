...
function MH(S0,n, r, T){
  //INPUT: the initial state object S0
  //       number of samples you want to make n
  //OUTPUT: array of states accepted by Metropolis-Hasting algorithm

  var StateArray = new Array(n+1)
  var q0to1 = 0
  var q1to0 = 0
  ...
  for (var i=1;i<n+1;i++){
    ...
    var Scand = GetCandidate(S0)
       
    // Calculate the probabilities
    q0to1 = GetProposalDist(StateArray[i-1],Scand)
    q1to0 = GetProposalDist(Scand,StateArray[i-1])
    RSD = getRSD(Scand,StateArray[i-1],r,T)    

    AcptProb = Math.min(1, RSD * q1to0/q0to1) 
    rand = Math.random()
...
var aspace = MH(SS, 10, 1, 1)
for (var i = 0;i<11;i++){
  console.log(aspace[i].AdjMatrix)
  console.log(' ')
}

module.exports = {State, getMinDist, getLenSP, getTheta, getRSD, IsConnected, CloneState, BridgeNumber, GetProposalDist, GetCandidate };
